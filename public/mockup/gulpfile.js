...
gulp.task('compile-styles',['fonts', 'scss-lint'], function (cb) {
  ...
  var compileStylesProcess = gulp.src([
    source + '/sass/*.scss',
    '!'+ source +'/sass/_*.scss'
  ])
  ...
  if (args.indexOf("--rails-assets") !== -1) {
    compileStylesProcess.pipe(replace(/url\(\"\.\.\/(fonts|images)\//gm, "asset-url(\""));
  }

  return compileStylesProcess.pipe(gulp.dest(dest + '/css'))
